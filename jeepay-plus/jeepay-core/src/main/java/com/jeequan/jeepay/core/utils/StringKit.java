...
public class StringKit {
     ...
     * @return
     */
    public static boolean checkInWhiteList(String clientIP, String whitelistStr) {
        whitelistStr = whitelistStr.replaceAll(System.lineSeparator(), "");
        String whitelist = whitelistStr.replaceAll(" ", "");
        if (clientIP.equalsIgnoreCase(whitelist)) {
            return true;
        }

        if (!"*".equals(whitelist)) {
            String[] ipList = whitelist.split("\\|");
            long count = Arrays.<String>stream(ipList).filter(ip -> StringUtils.equals(clientIP, ip)).count();
            if (count < 1L) {
                return false;
            }
        }
