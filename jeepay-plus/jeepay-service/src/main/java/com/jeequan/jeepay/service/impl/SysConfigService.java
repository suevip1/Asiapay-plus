...
public class SysConfigService extends ServiceImpl<SysConfigMapper, SysConfig> implements ISysConfigService {
     ...
     * 机器人相关设置
     */
    private static MutablePair<String, RobotsConfig> ROBOTS_CONFIG = new MutablePair<>("robotsConfigGroup", null);
    ...
    @Override
    public RobotsConfig getRobotsConfig() {
    ...
    public Byte getPassageAutoClearConfig() {
        Byte state = CS.NO;
        try {
            JSONObject params = this.selectByGroupKey(PAY_CONFIG.getLeft());
            Byte stateResult = params.getByte("payPassageAutoClean");
            if (stateResult == null) {
                return CS.NO;
            } else {
                state = stateResult;
     ...
     * @return
     */
    public boolean updatePassageAutoClearConfig(Byte state) {
        SysConfig sysConfig = new SysConfig();
        sysConfig.setConfigKey("payPassageAutoClean");
        sysConfig.setConfigVal(state.toString());
