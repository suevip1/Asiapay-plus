...
public class CurrentUserController extends CommonCtrl {
    ...
    public ApiRes modifyCurrentUserInfo() {
        ...
        sysUserAuthService.update(sysUserAuth,SysUserAuth.gw().eq(SysUserAuth::getAuthId,sysUserAuth.getAuthId()));
        //保存redis最新数据
...
//            isVerify = GoogleAuthUtil.CheckCode(sysUserAuth.getGoogleAuthKey(), googleCode);
    ...
    public ApiRes modifyPwd() throws BizException {
        ...
        sysUserAuthService.resetAuthInfo(getCurrentUser().getSysUser().getSysUserId(), opUserPwd, CS.SYS_TYPE.MGR);
        //调用登出接口
        return logout();
    }
