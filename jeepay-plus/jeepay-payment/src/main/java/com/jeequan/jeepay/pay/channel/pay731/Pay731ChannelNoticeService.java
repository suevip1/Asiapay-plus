...
@Slf4j
@Service
public class Pay731ChannelNoticeService extends AbstractChannelNoticeService {

    private static final String LOG_TAG = "[731支付]";
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        if (resultsParam != null) {
            //【md5(订单状态+商户号+商户订单号+支付金额+商户秘钥)】
            String state = jsonParams.getString("state");
            String merchantNum = resultsParam.getMchNo();

            String secret = resultsParam.getSecret();
            String signContent = state + merchantNum + orderNo + txnAmt + secret;
