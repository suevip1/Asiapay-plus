...
@Slf4j
@Service
public class LtfpayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("custSeq");        // 商户订单号
        ...
        if (resultsParam != null) {
            ...
            String signContentStr = SignatureUtils.getSignContent(map, null, null);
            String signContentStrA = SignatureUtils.md5(signContentStr).toLowerCase();
            final String signStr = SignatureUtils.md5(signContentStrA + secret).toLowerCase();
