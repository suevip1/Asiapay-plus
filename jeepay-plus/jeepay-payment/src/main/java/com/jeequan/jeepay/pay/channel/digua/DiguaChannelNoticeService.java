...
@Slf4j
@Service
public class DiguaChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            // 0支付中，1已完成，-1支付失败
            int status = jsonParams.getInteger("orderState");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String orderNo = jsonParams.getString("tradeNo");        // 商户订单号
        String txnAmt = jsonParams.getString("payFee");        // 支付金额
        ...
        map.remove("sign");
        //MD5(tradeNo+orderState+payFee+payTime+商户私钥)

        String tradeNo = jsonParams.getString("tradeNo");
        String orderState = jsonParams.getString("orderState");
        String payFee = jsonParams.getString("payFee");
        String payTime = jsonParams.getString("payTime");
        if (resultsParam != null) {
            String secret = resultsParam.getSecret();

            String signContentStr = tradeNo + orderState + payFee + payTime + secret;
