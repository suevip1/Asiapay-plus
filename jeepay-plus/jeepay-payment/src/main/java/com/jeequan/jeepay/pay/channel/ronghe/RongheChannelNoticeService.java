package com.jeequan.jeepay.pay.channel.ronghe;
...
public class RongheChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //订单状态：0 未支付 1 已支付 2订单超时 3下单异常
            String orderStatus = jsonParams.getString("orderStatus");

            if (!orderStatus.equals("1")) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, orderStatus);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("outTradeId");        // 商户订单号
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = SignatureUtils.getSignContentFilterEmpty(map, new String[]{"param"}) + "&key=" + secret;
