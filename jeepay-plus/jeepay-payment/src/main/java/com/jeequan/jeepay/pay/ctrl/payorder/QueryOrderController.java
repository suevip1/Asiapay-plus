...
public class QueryOrderController extends ApiController {
    ...
    @RequestMapping("/api/pay/queryOrderTest")
    public ApiRes queryOrderTest() {
        //返回商户号，订单号
        List<PayOrder> list = payOrderService.list(PayOrder.gw().select(PayOrder::getPayOrderId).eq(PayOrder::getState, PayOrder.STATE_ING));
