...
@Slf4j
@Service
public class HuanledouChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //订单状态：SUCCESS:支付成功,FAIL:支付失败
            String paySt = jsonParams.getString("paySt");

            if (!paySt.equals("SUCCESS")) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, paySt);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("mchntOrderNo");        // 商户订单号
        ...
        HuanledouParamsModel resultsParam = JSONObject.parseObject(payPassage.getPayInterfaceConfig(), HuanledouParamsModel.class);
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = SignatureUtils.getSignContentFilterEmpty(map, new String[]{"extra"}) + secret;
