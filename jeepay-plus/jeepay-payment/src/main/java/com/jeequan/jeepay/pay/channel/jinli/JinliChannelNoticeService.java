...
@Slf4j
@Service
public class JinliChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //	0：支付成功，其他值为失败
            int status = jsonParams.getInteger("opstate");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String txnAmt = jsonParams.getString("value");        // 支付金额
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = SignatureUtils.getSignContent(map, new String[]{"sign"}, null) + secret;
    ...
    public static void main(String [] args){
        String jsonStr = "{\n" +
                "    \"msg\": \"success\",\n" +
                "    \"merchantid\": \"80024601\",\n" +
                "    \"orderid\": \"P1734824220753514498\",\n" +
                "    \"sysorderid\": \"k2312131434545874410\",\n" +
                "    \"completetime\": \"2023/12/13 14:35:25\",\n" +
                "    \"sign\": \"e333719e98fc5226b8d273495f78948e\",\n" +
                "    \"attach\": \"\",\n" +
                "    \"opstate\": \"0\",\n" +
                "    \"value\": \"100.00\"\n" +
                "}";
        String secret = "fsXZ3a4Cc7r6EHSb36Gjxw26cTRTCW8n";
        final String signContentStr = SignatureUtils.getSignContent(JSONObject.parseObject(jsonStr), new String[]{"sign"}, null) + secret;
        final String signStr = SignatureUtils.md5(signContentStr).toUpperCase();
        log.info(signStr);
    }
