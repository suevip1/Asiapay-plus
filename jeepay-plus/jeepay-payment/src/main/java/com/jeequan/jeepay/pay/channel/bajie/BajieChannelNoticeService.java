...
@Slf4j
@Service
public class BajieChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //支付状态("TRADE_SUCCESS"表示成功)
            String trade_status = jsonParams.getString("trade_status");

            if (!trade_status.equals("TRADE_SUCCESS")) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, trade_status);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("code");        // 商户订单号
        ...
        if (resultsParam != null) {
            ...
            List<String> fieldOrder = Arrays.asList("username", "code", "price", "type", "extraParam");
            ...
            final String signStr = SignatureUtils.md5(signContent).toUpperCase();
