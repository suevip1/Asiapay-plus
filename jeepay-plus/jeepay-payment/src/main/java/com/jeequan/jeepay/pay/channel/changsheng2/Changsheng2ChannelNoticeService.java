...
@Slf4j
@Service
public class Changsheng2ChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            // 0：成功， 1：失败
            //请只处理这种情况！！！
            String status = jsonParams.getJSONObject("rdata").getString("pstatus");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getJSONObject("rdata").getString("outTradeNo");        // 商户订单号
        String txnAmt = jsonParams.getJSONObject("rdata").getString("totalFee");        // 支付金额
        ...
        String sign = jsonParams.getJSONObject("rdata").getString("sign");
        Map map = JSON.parseObject(jsonParams.getJSONObject("rdata").toJSONString());
        map.remove("sign");
