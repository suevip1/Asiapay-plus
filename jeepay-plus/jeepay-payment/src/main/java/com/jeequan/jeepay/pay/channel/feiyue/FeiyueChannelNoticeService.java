...
public class FeiyueChannelNoticeService extends AbstractChannelNoticeService {
    ...
    private static final BigDecimal FLOAT_AMOUNT = new BigDecimal("1.00");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        if (resultsParam != null) {
            ...
            if (signStr.equalsIgnoreCase(sign) && compareAmounts(orderAmount, channelNotifyAmount)) {
                return true;
            } else {
    ...
    // 浮动一元内
    public static boolean compareAmounts(BigDecimal amount1, BigDecimal amount2) {
        BigDecimal difference = amount2.subtract(amount1);
        return difference.abs().compareTo(FLOAT_AMOUNT) <= 0;
    }
