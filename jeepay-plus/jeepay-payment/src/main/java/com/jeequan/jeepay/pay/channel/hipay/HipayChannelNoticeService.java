...
import com.jeequan.jeepay.pay.channel.hengsheng.HengshengParamsModel;
...
@Slf4j
@Service
public class HipayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        JSONObject respData = jsonParams.getJSONObject("data");

        String orderNo = respData.getString("orderid");        // 商户订单号
        String txnAmt = respData.getString("amount");        // 支付金额
        ...
        String resign = respData.getString("resign");

        if (normalMchParams != null) {
            String key = normalMchParams.getSecret();
            final String signContentStr = SignatureUtils.md5(sign + key);
            if (resign.equalsIgnoreCase(signContentStr) && orderAmount.compareTo(channelNotifyAmount) == 0) {
                return true;
            } else {
