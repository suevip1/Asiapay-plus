...
public class BigDecimalUtil {
    ...
    /**
     * 至少两个数值相除，除数不能为0，默认结果四舍五入保留2位小数
     * <p>number1 / number2 / ... / number(n)</p>
    ...
    public BigDecimal divideTry(Object number1, Object number2) {
        try {
            return new BigDecimal(number1.toString()).divide(new BigDecimal(number2.toString()), scale, roundingMode);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public BigDecimal divideWithJudge(Object number1, Object number2, BigDecimal ifDivisorNullReturn) {
        ...
        return new BigDecimal(number1.toString()).divide(bg2, scale, roundingMode);
    }
    ...
    /** 保留小数位 */
    private int scale;
    /** 向零方向舍入的舍入模式(此舍入模式始终不会增加计算值的绝对值) */
