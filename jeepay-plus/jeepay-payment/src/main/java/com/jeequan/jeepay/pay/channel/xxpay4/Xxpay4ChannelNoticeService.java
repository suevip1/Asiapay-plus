...
@Slf4j
@Service
public class Xxpay4ChannelNoticeService extends AbstractChannelNoticeService {

    private static final String LOG_TAG = "[xxpay4支付]";

    private static final String ON_FAIL = "fail";

    private static final String ON_SUCCESS = "{\"code\":200}";
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            // 0.订单生成、1.待支付、2.支付成功、3.支付失败、4.处理完成、5.已退款、6.订单失效关闭
            //注释：开发者收到代收订单回调时，status == 2 表示订单已经支付成功了，开发者回调JSON {"code":200} 订单状态会变成status == 4 处理完成状态
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        boolean within10Percent = isWithin10Percent(channelNotifyAmount, orderAmount);
        ...
        if (resultsParam != null) {
            ...
            if (signStr.equalsIgnoreCase(sign) && within10Percent) {
                return true;
            } else {
    ...
    private boolean isWithin10Percent(BigDecimal num1, BigDecimal num2) {
        BigDecimal tenPercent = num1.multiply(new BigDecimal("0.10"));
        BigDecimal difference = num1.subtract(num2).abs();

        return difference.compareTo(tenPercent) <= 0;
    }
}