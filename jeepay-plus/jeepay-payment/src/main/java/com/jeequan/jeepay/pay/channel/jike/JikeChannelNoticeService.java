...
@Slf4j
@Service
public class JikeChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //订单状态：0 待支付 100 支付成功 -90 支付失败
            String PayOrderStatus = jsonParams.getString("PayOrderStatus");

            if (!PayOrderStatus.equals("100")) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, PayOrderStatus);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("MerchantUniqueOrderId");        // 商户订单号
        String txnAmt = jsonParams.getString("RealAmount");        // 支付金额
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = SignatureUtils.getSignContent(map, null, null) + secret;
