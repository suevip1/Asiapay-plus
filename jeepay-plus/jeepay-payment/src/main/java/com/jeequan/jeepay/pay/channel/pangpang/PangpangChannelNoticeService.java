...
@Slf4j
@Service
public class PangpangChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //pay_result=1时为支付成功标识
            int status = jsonParams.getInteger("pay_result");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("pay_num");        // 商户订单号
        ...
        String pay_num = jsonParams.getString("pay_num");
        String pay_amount = jsonParams.getString("pay_amount");
        String today = jsonParams.getString("today");
        if (resultsParam != null) {
            String secret = resultsParam.getSecret();

            final String signContentStr = merchant_no + pay_num + pay_amount + today + secret;
