...
@Slf4j
@Service
public class WumingChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("oid");        // 商户订单号
        ...
        int code = jsonParams.getInteger("code");
        String msg = jsonParams.getString("msg");
        String mid = jsonParams.getString("mid");
        String pt_oid = jsonParams.getString("pt_oid");
        String oid = jsonParams.getString("oid");
        String out_oid = jsonParams.getString("out_oid");
        int type = jsonParams.getInteger("type");
        int pay_type = jsonParams.getInteger("pay_type");
        String account = jsonParams.getString("account");
        long price = jsonParams.getLong("price");
        int status = jsonParams.getInteger("status");
        String timeout = jsonParams.getString("timeout");
        String timestamp = jsonParams.getString("timestamp");
        int rand = jsonParams.getInteger("rand");
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = code + msg + mid + pt_oid + oid + out_oid + type + pay_type + account + price + status + timeout + timestamp + rand + secret;
