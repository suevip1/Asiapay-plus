...
public class TangbangChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            int status = jsonParams.getJSONObject("data").getInteger("status");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String txnAmt = jsonParams.getJSONObject("data").getString("realMoney");        // 支付金额
        ...
        String sign = jsonParams.getJSONObject("data").getString("sign");
        Map map = JSON.parseObject(jsonParams.getJSONObject("data").toJSONString());
        map.remove("sign");
        if (tangBangParamsModel != null) {
            String aesSecret = tangBangParamsModel.getAesSecret();

            // AES解密
            String stringAes = JSON.toJSONString(map);
            String signStr = SignatureUtils.decryptAES(stringAes, aesSecret, null);
