...
@Slf4j
@Service
public class KxpayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //status, 状态，0待支付，2已支付但未回调，3已支付且已回调，9取消支付。2 与 3 都表示该订单已支付
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("transactionId");        // 商户订单号
        ...
        if (resultsParam != null) {
            ...
            String signContentStr = secret + "&" + SignatureUtils.getSignContentFilterEmpty(map, null);
