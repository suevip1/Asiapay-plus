...
@Slf4j
@Service
public class XiqueChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        map.remove("sign");
        if (xiqueParamsModel != null) {
            String secret = xiqueParamsModel.getSecret();
            String publicKey = xiqueParamsModel.getRequestPublicKey();
            ...
            boolean verifySign = SignatureUtils.buildSHA256WithRSAVerifyByPublicKey(signStr, publicKey, sign);
