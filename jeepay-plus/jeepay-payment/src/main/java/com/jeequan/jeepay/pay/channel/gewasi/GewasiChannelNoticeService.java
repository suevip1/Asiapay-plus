...
@Slf4j
@Service
public class GewasiChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //payStatus 状态： W（等待支付）、Y(成功)、N(失败)
            String status = jsonParams.getString("payStatus");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        if (resultsParam != null) {
            ...
            final String signStr = SignatureUtils.md5(SignatureUtils.md5(JeepayKit.getSign(map, secret).toLowerCase())).toLowerCase();
