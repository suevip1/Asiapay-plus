...
@Slf4j
@Service
public class GalipayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //1003 已付款
            //1009 已过期
            int status = jsonParams.getInteger("ordidState");

            if (status != 1003) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, status);
                result.setChannelState(ChannelRetMsg.ChannelState.CONFIRM_FAIL);
                result.setChannelOrderId(jsonParams.getString("bankOrderId"));
            } else {
                //验签成功后判断上游订单状态
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String txnAmt = jsonParams.getString("payMoney");        // 支付金额
        ...
        String ordidState = map.get("ordidState").toString();
        String bankOrderId = map.get("bankOrderId").toString();
        String merOrderId = map.get("merOrderId").toString();
        String selfParam = map.get("selfParam").toString();

        Map<String, Object> mapLink = new LinkedHashMap<>();

        mapLink.put("payId", payId);
        mapLink.put("merId", merId);
        mapLink.put("payTime", payTime);
        mapLink.put("payMoney", payMoney);
        mapLink.put("ordidState", ordidState);
        mapLink.put("bankOrderId", bankOrderId);
        mapLink.put("merOrderId", merOrderId);
        mapLink.put("selfParam", selfParam);

        JSONObject dataJSON = new JSONObject(mapLink);

        if (resultsParam != null) {
            String secret = resultsParam.getSecret();

            String signContentStr = dataJSON.toJSONString() + secret;
