...
@Slf4j
@Service
public class XiaojiChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //支付状态,200 成功
            int status = JSONObject.parseObject(rawQuery).getInteger("code");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String attach = jsonParams.getString("attach");
        String type = jsonParams.getString("type");
        String really_amount = jsonParams.getString("really_amount");
        String mch_id = resultsParam.getMchNo();
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = orderNo + attach + type + txnAmt + really_amount + mch_id + secret;
