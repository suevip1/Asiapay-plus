...
@Slf4j
@Service
public class CyoupayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("sfOrderId");        // 商户订单号
        ...
        String sfOrderId = map.get("sfOrderId").toString();
        String money = map.get("money").toString();

        if (resultsParam != null) {
            String secret = resultsParam.getSecret();
//            sign=zLDVbutkkukLlAx8&status=1&money=100&sfOrderId=202310191708054802395&time=1708054808395
            String signContentStr = "sign=" + secret + "&status=" + status + "&money=" + money + "&sfOrderId=" + sfOrderId + "&time=" + time;
