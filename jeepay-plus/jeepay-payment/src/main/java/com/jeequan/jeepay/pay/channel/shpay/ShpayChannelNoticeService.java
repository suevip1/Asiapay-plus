...
@Slf4j
@Service
public class ShpayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //只有3为成功
            int state = jsonParams.getInteger("status");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String orderNo = jsonParams.getString("shOrder");        // 商户订单号
        ...
        String shOrder = jsonParams.getString("shOrder");
        String pOrder = jsonParams.getString("pOrder");
        ...
        if (resultsParam != null) {
            ...
            String signContentStr = "shOrder=" + shOrder + "&pOrder=" + pOrder + "&money=" + money + "&status=" + status + "&time=" + time + "&key=" + secret;
