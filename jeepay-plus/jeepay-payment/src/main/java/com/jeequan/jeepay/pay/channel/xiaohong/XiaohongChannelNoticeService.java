...
@Slf4j
@Service
public class XiaohongChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //是否支付成功 1代表支付成功 其余均失败
            int is_pay = jsonParams.getInteger("is_pay");

            if (is_pay != 1) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, is_pay);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        if (resultsParam != null) {
            ...
            final String signContentStr = SignatureUtils.getSignContentFilterEmpty(map, null) + "&appkey=" + secret;
