...
public class PppayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        String retsign = jsonParams.getString("retsign");
        if (resultsParam != null) {
            String secret = resultsParam.getSecret();

            final String signContentStr = sign + secret;
            final String signStr = SignatureUtils.md5(signContentStr).toLowerCase();
            if (signStr.equalsIgnoreCase(retsign) && orderAmount.compareTo(channelNotifyAmount) == 0) {
                return true;
            } else {
