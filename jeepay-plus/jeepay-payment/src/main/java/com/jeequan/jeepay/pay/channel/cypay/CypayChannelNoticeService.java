...
@Slf4j
@Service
public class CypayChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //订单状态,(1-成功)
            int status = jsonParams.getInteger("orderStatus");

            if (status != 1) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, status);
                result.setChannelState(ChannelRetMsg.ChannelState.CONFIRM_FAIL);
                result.setChannelOrderId(jsonParams.getString("payOrderId"));
            } else {
                //验签成功后判断上游订单状态
