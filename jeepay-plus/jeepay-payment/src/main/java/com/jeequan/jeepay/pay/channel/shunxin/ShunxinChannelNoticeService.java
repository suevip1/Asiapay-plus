...
@Slf4j
@Service
public class ShunxinChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //支付状态,订单已支付才会回调,此参数值固定为 ok
            int state = jsonParams.getInteger("state");
            int payState = jsonParams.getInteger("payState");

            if (state == 0 && payState == 0) {
                //验签成功后判断上游订单状态
                result.setChannelState(ChannelRetMsg.ChannelState.CONFIRM_SUCCESS);
            } else {
                log.info("[{}]回调通知订单状态错误:{},{}", LOG_TAG, state, payState);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        if (resultsParam != null) {
            ...
            String signContent = SignatureUtils.getSignContent(map, null, null) + "&key=" + secret;
