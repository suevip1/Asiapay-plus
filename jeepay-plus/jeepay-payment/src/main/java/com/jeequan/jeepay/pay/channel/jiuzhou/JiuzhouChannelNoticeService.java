...
@Slf4j
@Service
public class JiuzhouChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        JSONObject jsonData = jsonParams.getJSONObject("data");

        String orderNo = jsonData.getString("orderNo");        // 商户订单号
        String txnAmt = jsonData.getString("amount");        // 支付金额
        ...
        Map<String, Object> map = JSON.parseObject(jsonData.toJSONString());

        if (resultsParam != null) {
            String secret = resultsParam.getSecret();

            String signContentStr = SignatureUtils.getSignContentFilterEmpty(map, null) + "&key=" + secret;
