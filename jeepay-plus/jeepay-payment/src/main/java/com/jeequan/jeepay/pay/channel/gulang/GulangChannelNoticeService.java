...
@Slf4j
@Service
public class GulangChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            jsonParams = jsonParams.getJSONObject("data");
            // 校验支付回调
            ...
            //支付状态：成功：succeeded 失败：failed 超时未支付 expired 支付中：inprogress
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        ...
        map.remove("sign");
        if (gulangParams != null) {
            String secret = gulangParams.getSecret();
            final String signContentStr = SignatureUtils.getSignContentFilterEmpty(map, null);
            final String signStr = SignatureUtils.hmacSign(secret, signContentStr);
