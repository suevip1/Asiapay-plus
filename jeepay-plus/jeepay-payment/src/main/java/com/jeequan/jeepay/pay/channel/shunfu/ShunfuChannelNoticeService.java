...
@Slf4j
@Service
public class ShunfuChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            if (!Pay_Status.equals("0")) {
                log.info("[{}]回调通知订单状态错误:{}", LOG_TAG, Pay_Status);
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("Pay_Ord");        // 商户订单号
        ...
        // 回调签名参数为四个，Pay_Amount，Pay_MerID，Pay_Ord，Pay_Status 将这4个参数按字典序排序参数
        Map<String, Object> signMap = new HashMap<>();
        signMap.put("Pay_Amount", map.get("Pay_Amount"));
        signMap.put("Pay_MerID", map.get("Pay_MerID"));
        signMap.put("Pay_Ord", map.get("Pay_Ord"));
        signMap.put("Pay_Status", map.get("Pay_Status"));
