...
@Slf4j
@Service
public class FeicuiChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder, PayPassage payPassage, NoticeTypeEnum noticeTypeEnum) {
        ...
        try {
            ...
            //订单状态（NP：等待支付；AP：订单成功；AF：订单失败；AC：订单关闭）
            String state = jsonParams.getString("orderStatus");
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("callerOrderId");        // 商户订单号
        ...
        if (resultsParam != null) {
            ...
            final String signStr = FeicuiPaymentService.sign(map, secret);
