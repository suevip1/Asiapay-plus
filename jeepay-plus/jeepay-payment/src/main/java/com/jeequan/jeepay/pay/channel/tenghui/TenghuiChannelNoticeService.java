...
@Slf4j
@Service
public class TenghuiChannelNoticeService extends AbstractChannelNoticeService {
    ...
    public boolean verifyParams(JSONObject jsonParams, PayOrder payOrder, PayPassage payPassage) {
        String orderNo = jsonParams.getString("mch_order_no");        // 商户订单号
        String txnAmt = jsonParams.getString("pay_money");        // 支付金额
        ...
        // 回调签名的字段（mch_id,order_no,mch_order_no,pay_money,time,code）
        Map<String, Object> signMap = new HashMap<>();
        signMap.put("mch_id", map.get("mch_id"));
        signMap.put("order_no", map.get("order_no"));
        signMap.put("mch_order_no", map.get("mch_order_no"));
        signMap.put("pay_money", map.get("pay_money"));
        signMap.put("time", map.get("time"));
        signMap.put("code", map.get("code"));
        if (resultsParam != null) {
            String secret = resultsParam.getSecret();
            final String signContentStr = SignatureUtils.getSignContentFilterEmpty(signMap, null) + "&key=" + secret;
            final String firstSignStr = SignatureUtils.md5(signContentStr).toLowerCase();
            final String signStr = SignatureUtils.md5(firstSignStr + secret).toUpperCase();
