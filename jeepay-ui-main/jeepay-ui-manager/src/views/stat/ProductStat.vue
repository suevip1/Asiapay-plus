  ...
  <page-header-wrapper>
    <a-card style="margin-top: 20px">
      ...
      <JeepayTable
          ...
          :pageSize="100"
          rowKey="statisticsProductId"
      >
        <template slot="productTotalSlot" slot-scope="{record}">
          <b>{{ ((record.totalAmount)/ 100).toFixed(2) }}</b>
        </template> <!-- 自定义插槽 -->
...
import { API_URL_MGR_PRODUCT_STAT, API_URL_PAYWAYS_LIST, req } from '@/api/manage'
...
const tableColumns = [
  ...
  { key: 'amount', width: '200px', title: '成交金额(￥)', scopedSlots: { customRender: 'amountSlot' } },
  {
    ...
    title: '订单总额(￥)',
    width: '200px',
    scopedSlots: { customRender: 'productTotalSlot' }
  },
  {
    key: 'successRate',
    title: '支付成功率',
...
export default {
  ...
  methods: {
    ...
    reqTableDataFunc: (params) => {
      return req.list(API_URL_MGR_PRODUCT_STAT, params)
    },
