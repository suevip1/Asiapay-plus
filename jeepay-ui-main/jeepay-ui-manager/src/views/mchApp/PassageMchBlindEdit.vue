    ...
    <div class="table-page-search-wrapper" @keydown.enter="queryFunc">
      <a-form layout="inline" class="table-head-ground">
        <a-row justify="space-between" type="flex">
          <a-col :sm="12">
            <a-descriptions>
              <a-descriptions-item label="通道信息">
                <b style="color: #1A79FF">[{{ selectPassage.payPassageId }}]</b><b>{{ selectPassage.payPassageName }}</b>
              </a-descriptions-item>
            </a-descriptions>
          </a-col>
          <a-col :sm="12">
            <a-descriptions>
              <a-descriptions-item label="通道费率">
                <a-tag color="blue">{{ (selectPassage.rate * 100).toFixed(2)}}%</a-tag>
              </a-descriptions-item>
            </a-descriptions>
          </a-col>
        </a-row>
...
export default {
  ...
  methods: {
    ...
    reqTableDataFunc: (params) => {
      return req.list(API_URL_PASSAGE_MCH_LIST, params)
    },
    blindSelect: function (record) {
      const params = { }
      params.payPassageId = this.selectPassage.payPassageId
      params.state = 1
      params.mchNo = record.mchNo
      req.updateById(API_URL_PASSAGE_MCH_LIST, '', params).then(res => {
        this.$refs.mchPassageTable.refTable(true)
    ...
    blindAll () {
      this.btnLoading = true
      req.postNormal(API_URL_PASSAGE_MCH_LIST + '/blindAll', this.selectPassage.payPassageId).then(res => {
        setTimeout(() => {
    ...
    unBlindAll () {
      this.btnLoading = true
      req.postNormal(API_URL_PASSAGE_MCH_LIST + '/unBlindAll', this.selectPassage.payPassageId).then(res => {
        setTimeout(() => {
          this.btnLoading = false
          this.$refs.mchPassageTable.refTable(true)
          this.$message.success('修改成功')
