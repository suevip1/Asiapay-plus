    ...
    <div class="chart-bottom">
      <div class="chart-item bottom-left">
        <div class="chart-data">
          ...
          <a-radio-group v-model="showType" button-style="solid" @change="selectType">
            <a-radio-button value="1">数据排名</a-radio-button>
            <a-radio-button value="2">商户并发</a-radio-button>
          </a-radio-group>
          <div v-show="!skeletonIsShow">
            ...
            <div v-show="showType==='2'">
              <MchConCurrent ref="mchConCurrent"></MchConCurrent>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="chart-item bottom-right">
        <div class="chart-data">
          <div v-show="!skeletonIsShow">
            <div class="pay-count-title">
              <span class="chart-title" style="font-weight: bold;color: #262626;font-size: 18px">通道监控</span>
              <span style="color: #969B9F">
                <a-radio-group v-model="tableTime" button-style="solid" @change="selectPassageTab">
...
import MchConCurrent from '@/views/dashboard/MchConCurrent.vue'
...
export default {
  ...
  components: { JeepayTable, JeepayTableColumns, empty, MchConCurrent },
  methods: {
    init () {
      const that = this
    ...
    initChart () { // 初始化统计表
      ...
      this.passageData = []
      const that = this
      getRealTimeStat(this.tableTime).then(res => {
        // 在这里配置你的图表选项和数据
        const yAxisData = []
        for (var key in res) {
