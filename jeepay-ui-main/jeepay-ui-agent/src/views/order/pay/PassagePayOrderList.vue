...
<!--            <jeepay-text-up :placeholder="'通道名'" :msg="searchData.payPassageName" v-model="searchData.payPassageName" />-->
      ...
      >
        <template slot="mchSlot" slot-scope="{record}">
          <span style="color: #1A79FF">[{{ record.passageId }}]</span>&nbsp;<span>{{ record.ext.payPassageName }}</span>
        </template> <!-- 商户信息插槽 -->
    ...
    <template>
      ...
      >
        <a-row justify="space-between" type="flex">
          ...
          <a-divider />
          <a-col :sm="12">
            <a-descriptions><a-descriptions-item label="代理费率"><b>{{ (detailData.agentPassageRate * 100).toFixed(2) }}%</b></a-descriptions-item></a-descriptions>
          </a-col>
          <a-col :sm="12">
            <a-descriptions><a-descriptions-item label="代理分润"><a-tag color="pink">{{ detailData.agentPassageFee / 100 }}</a-tag></a-descriptions-item></a-descriptions>
          </a-col>
          <a-divider />
          <a-col :sm="12">
            <a-descriptions>
...
export default {
  ...
  methods: {
    ...
    reqTableDataFunc: (params) => {
      return req.list(API_URL_PASSAGE_PAY_ORDER_LIST, params)
    },
    ...
    detailFunc: function (recordId) {
      const that = this
      req.getById(API_URL_PASSAGE_PAY_ORDER_LIST, recordId).then(res => {
        that.detailData = res
      })
      this.visible = true
    },
    ...
    exportExcel: function () { // todo 这里返回的结果没有嵌入到通用返回格式中
      exportExcel('/api/payOrderExport/exportPassageExcel', this.searchData).then(res => {
        const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        // 使用 FileSaver.js 的 saveAs 方法将 Blob 对象保存为文件
        saveAs(blob, moment().format('YYYY-MM-DD') + '-通道代理订单流水.xlsx')
      }).catch((resErr) => {
