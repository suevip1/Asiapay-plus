...
router.beforeEach((to, from, next) => {
  ...
  if (!store.state.user.userId) {
    ...
    getInfo().then(bizData => {
      // todo 这里为什么会有ext字段
      if (bizData.ext !== undefined) {
        bizData.entIdList = bizData.ext.entIdList
        bizData.allMenuRouteTree = bizData.ext.allMenuRouteTree
        bizData.googleAuth = bizData.ext.googleAuth
        bizData.ext = {}
      }
